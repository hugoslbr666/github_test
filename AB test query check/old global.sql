...
WITH non_intra AS (
  SELECT
    ...
    when variation_id = 1 then 'Test' END AS version,
  ...
  INNER JOIN `singulart-db-to-bigquery.singulartdb.sgt_tracking_visitors` stv ON stv.id = ge.visitor_id
  INNER JOIN `singulart-db-to-bigquery.singulartdb.sgt_tracking_visitors_experiments` stve ON stve.visitor_id = ge.visitor_id
  ...
  WHERE ge.event_date >= "2026-02-25"
    ...
    AND server_side_experiment_id = 211
    AND stve.created_at <= event_date
  GROUP BY 1
), intra AS (
  SELECT 
    CASE 
