...
WITH 
 homepage_sessions AS (
  ...
  AND tpl IN ('homepage', 'homepage_raw')
  GROUP BY ALL
),
...
non_intra AS (
  SELECT
    CASE
      WHEN ge.ss_exp LIKE "%219.0%" THEN "Control"
      WHEN ge.ss_exp LIKE "%219.1%" THEN "Test"
      ELSE "?"  
    END AS version,
    ...
    COUNT(DISTINCT IF(ge.event_name = "purchase" AND ge.event_value_in_usd <= 10000, ge.new_eventId, NULL)) AS nb_purchase_less10kUSD,
    ...
    COUNT(DISTINCT IF(ge.event_name = "purchase" AND ge.event_value_in_usd <= 10000, ge.ga_session_id, NULL)) AS nb_sessions_purchase_less10kUSD,
    ...
    SUM(IF(ge.event_name = "purchase" AND ge.event_value_in_usd <= 10000, ge.event_value_in_usd*scr.rate, 0)) AS bv_eur_estimate_less10kUSD,
  ...
  INNER JOIN homepage_sessions hs ON hs.session_id = ge.sg_session_id
  -- LEFT JOIN UNNEST(ge.items) AS items a été supprimé ici
  INNER JOIN `singulart-db-to-bigquery.singulartdb.sgt_tracking_visitors` stv 
      ON stv.id = ge.visitor_id
  INNER JOIN `singulart-db-to-bigquery.singulartdb.sgt_currencies_rates` scr 
      ON scr.base_id = 142 AND scr.target_id = 43
  WHERE ge.event_date >= "2026-05-07"
    AND ge.ga_session_id IS NOT NULL
    AND ge.ss_exp LIKE "%219%"
  GROUP BY 1
), intra AS (
  SELECT 
    CASE
...
SELECT
  ...
  SUM(nb_purchase_less10kUSD) AS nb_purchase_less10kUSD,
  ...
  SUM(nb_sessions_purchase_less10kUSD) AS nb_sessions_purchase_less10kUSD,
  --SUM(nb_sessions_newsletter_subscription) AS nb_sessions_newsletter_subscription,
  SUM(bv_eur_estimate) AS bv_eur_estimate,
  SUM(bv_eur_estimate_less10kUSD) AS bv_eur_estimate_less10kUSD,
  ...
  SAFE_DIVIDE(SUM(bv_eur_estimate_less10kUSD), SUM(nb_sg_visitor_id)) AS mean_bv_per_visitor_less10kUSD,
