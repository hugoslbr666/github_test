...
prices as (
  ...
  *,
  ...
  select
  *, 
  row_number() over(partition by id order by current_period_start desc) as rn
...
row_number() over(partition by sap.artist_id order by sub.batch_timestamp asc) as rn
...
select
* ,
lag(canceled_at) over(partition by artist_id order by rn) as prev_cancelled_at,
lag(cancel_at) over(partition by artist_id order by rn) as prev_cancel_at,
case when rn = 1 then 'subscription_creation'
else lag(status) over(partition by artist_id order by rn) end as prev_status
from sq
),
...
case when
prev_status = 'past_due' then lag(coalesce(w1.payment_failed_at,data_batch_date)) over(partition by w1.artist_id order by w1.rn) else null end as prev_payment_failed_at,
lag(w1.current_period_end) over(partition by w1.artist_id order by w1.rn)
as prev_period_end,
/*
-- activation issue
...
case
  ...
  when w1.status not in ('canceled') and cancellation_details_reason in ('cancellation_requested') then 1
  else 0 end
as is_canceller,
-- new cancellation event
case
  when canceled_at is not null and w1.status = 'active' and cancellation_details_reason not in ('payment_failed') and (coalesce(prev_cancelled_at,prev_cancel_at) is null) then 1
  when canceled_at is not null and w1.status = 'canceled' and cancellation_details_reason not in ('payment_failed') and w1.prev_status = 'active' and (coalesce(prev_cancelled_at,prev_cancel_at) is null) then 1
  when w1.status not in ('canceled') and cancellation_details_reason in ('cancellation_requested') and (coalesce(prev_cancelled_at,prev_cancel_at) is null) then 1
else 0 end
as cancellation_event,

-- canceller retention event
case
  when coalesce(canceled_at, cancel_at) is null and coalesce(prev_cancelled_at, prev_cancel_at) is not null then 1
  else 0 end
...
row_number() over (partition by processing.id, data_batch_timestamp order by data_batch_timestamp desc, hubspot_data.create_date asc) as final_dedup
from processing
...
left join `singulart-datasandbox.hugo.canceller_deal_manual_corrections` cdmc on cdmc.artist_id = final.artist_id and date(cancellation_due_date) = date(final.cancel_at)
where final.artist_id is not null and
