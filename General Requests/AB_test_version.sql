...
WITH non_intra AS (
  SELECT
    CASE
      WHEN ge.ss_exp LIKE "%220.0%" THEN "Control"
      WHEN ge.ss_exp LIKE "%220.1%" THEN "Test"
      ELSE "?"
    END AS version,
    ...
    COUNT(DISTINCT IF(ge.event_name = "ArtistFollowReady", ge.new_eventId, NULL)) AS nb_follow_artists,
    ...
    COUNT(DISTINCT IF(ge.event_name = "ArtistFollowReady", ge.ga_session_id, NULL)) AS nb_sessions_follow_artists,
  ...
  WHERE ge.event_date >= "2026-06-10"
    AND ge.ga_session_id IS NOT NULL
    AND ge.ss_exp LIKE "%220%"
  GROUP BY 1
), intra AS (
  SELECT 
    CASE
...
SELECT
  ...
  SUM(nb_follow_artists) AS nb_follow_artists,
  ...
  SUM(nb_sessions_follow_artists) AS nb_sessions_follow_artists,
