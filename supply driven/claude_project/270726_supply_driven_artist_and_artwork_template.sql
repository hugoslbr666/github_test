...
SELECT
    aa.artist_id,
    aa.artist_name,
    aa.status as artist_status,
    aa.email as artist_email,
    ...
    CASE
        WHEN aa.status IN ('LIVE_ONLINE', 'ONBOARDING') THEN DATE_DIFF(CURRENT_DATE(), DATE(aa.online_at), YEAR)
        WHEN aa.status IN ('LIVE_OFFLINE', 'NEVER_LOGGED_IN') THEN DATE_DIFF(DATE(aa.updated_at), DATE(aa.online_at), YEAR)
        ELSE NULL
